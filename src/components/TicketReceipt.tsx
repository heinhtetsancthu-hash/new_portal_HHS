...
import { getStoredTerms } from './Settings';
...
export const TicketReceipt = React.forwardRef<HTMLDivElement, TicketReceiptProps>(({ ticket }, ref) => {
  ...
  return (
    ...
    >
      ...
      {/* Details Box */}
      <div className="bg-[#f8fafc] rounded-2xl p-6 border border-[#e2e8f0] mb-8" style={{ backgroundColor: '#f8fafc', borderColor: '#e2e8f0' }}>
        ...
        <div className="mt-6">
           <p className="text-[10px] font-bold text-[#94a3b8] uppercase tracking-wider mb-2">Service Faults / Note</p>
           <div className="bg-[#ffffff] border border-[#e2e8f0] rounded-xl p-4 text-sm text-[#334155] min-h-[60px]" style={{ backgroundColor: '#ffffff', borderColor: '#e2e8f0' }}>
             {ticket.notes || '-'}
           </div>
        </div>
      </div>
      ...
      {/* Terms */}
      <div className="bg-[#fefce8] border border-[#fef08a] rounded-xl p-5 relative" style={{ backgroundColor: '#fefce8', borderColor: '#fef08a' }}>
         <div className="flex items-start gap-2 mb-2">
            <Info size={14} className="text-[#854d0e] mt-0.5" />
            <p className="text-xs font-bold text-[#854d0e]">Terms & Conditions:</p>
         </div>
         <div className="text-[10px] text-[#713f12] whitespace-pre-wrap leading-relaxed pl-5 font-medium">
            {terms}
         </div>
      </div>
    </div>
